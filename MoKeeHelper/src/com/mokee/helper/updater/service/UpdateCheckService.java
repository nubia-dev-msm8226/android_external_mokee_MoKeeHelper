...
public class UpdateCheckService extends IntentService
	{
		...
		protected void onHandleIntent(Intent intent)
			{
				...
				LinkedList<MokeeUpdateInfo> availableUpdates;
				try
					{
				...
				PreferenceManager.getDefaultSharedPreferences(UpdateCheckService.this).edit()
						.putLong(Constants.LAST_UPDATE_CHECK_PREF, d.getTime()).putBoolean(Constants.BOOT_CHECK_COMPLETED, true)
						.apply();
				...
				if (realUpdateCount != 0 && !app.isMainActivityActive())
					{
						...
						Intent i = new Intent(this, MoKeeCenter.class);
						...
						Notification.Builder builder = new Notification.Builder(this).setSmallIcon(R.drawable.cm_updater)
						...
						LinkedList<MokeeUpdateInfo> realUpdates = new LinkedList<MokeeUpdateInfo>();
						...
						if(!PreferenceManager.getDefaultSharedPreferences(this).getBoolean(Constants.PREF_ROM_OTA, true))//ota暂时不进行排序
							{
								Collections.sort(realUpdates, new Comparator<MokeeUpdateInfo>()
											{
												@Override 
						...
						Notification.InboxStyle inbox = new Notification.InboxStyle(builder).setBigContentTitle(text);
						int added = 0, count = realUpdates.size();

						for (MokeeUpdateInfo ui : realUpdates)
							{
						...
						if (added != count)
							{
								inbox.setSummaryText(res.getQuantityString(R.plurals.not_additional_count, count - added, count
										- added));
							}
						...
						if (count == 1)
							{
								...
								PendingIntent downloadIntent = PendingIntent.getBroadcast(this, 0, i, PendingIntent.FLAG_ONE_SHOT
		...
		private LinkedList<MokeeUpdateInfo> getMKAvailableUpdatesAndFillIntent(Intent intent) throws IOException
			{
				...
				if (!isOTA)
					{
						...
						// params.add(new BasicNameValuePair("device_version", "MK43.1-edison-131106-RELEASE.zip"));
				...
				LinkedList<MokeeUpdateInfo> lastUpdates = State.loadMKState(this);
				// Read the ROM Infos
				String json = EntityUtils.toString(entity, "UTF-8");
				System.out.println(json);
				LinkedList<MokeeUpdateInfo> updates = parseMKJSON(json, updateType,isOTA);
		...
		private LinkedList<MokeeUpdateInfo> parseMKJSON(String jsonString, int updateType,boolean isOTA)
			{
				LinkedList<MokeeUpdateInfo> updates = new LinkedList<MokeeUpdateInfo>();
				try
					{
						JSONArray[] jsonArrays = new JSONArray[2];
						if (!isOTA&&updateType == Constants.UPDATE_TYPE_ALL)// 判断全部
							{
										...
										for (int j = 0; j < jsonArray.length(); j++)
											{
												...
												MokeeUpdateInfo info = parseUpdateMKJSONObject(item, updateType);
												if (info != null)
													{
														updates.add(info);
													}
		...
		private MokeeUpdateInfo parseUpdateMKJSONObject(JSONObject obj, int updateType) throws JSONException
			{
				String name = obj.getString("name");
				String rom = obj.getString("rom");
				String md5 = obj.getString("md5");
				String log = obj.getString("log");
				MokeeUpdateInfo mui = new MokeeUpdateInfo(log, md5, name, rom);
		...
		private void fetchMkChangeLog(MokeeUpdateInfo info, String url)
			{
