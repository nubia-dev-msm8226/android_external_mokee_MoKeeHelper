...
public class DownloadReceiver extends BroadcastReceiver
	{
		...
		public void onReceive(Context context, Intent intent)
			{
				...
				if (ACTION_START_DOWNLOAD.equals(action))
					{
						MokeeUpdateInfo ui = (MokeeUpdateInfo) intent.getParcelableExtra(EXTRA_UPDATE_INFO);
		...
		private void handleStartDownload(Context context, SharedPreferences prefs, MokeeUpdateInfo ui)
			{
				...
				String fullFilePath = "file://" + directory.getAbsolutePath() + "/" + ui.getName() + ".partial";
				...
				prefs.edit().putLong(Constants.DOWNLOAD_ID, downloadId).putString(Constants.DOWNLOAD_MD5, ui.getMd5()).apply();
		...
		private void handleDownloadComplete(Context context, SharedPreferences prefs, long id)
			{
				...
				if (status == DownloadManager.STATUS_SUCCESSFUL)
					{
						...
						String partialFileFullPath = c.getString(c.getColumnIndex(DownloadManager.COLUMN_LOCAL_FILENAME));
						...
						if (MD5.checkMD5(downloadedMD5, updateFile))
							{
								...
								updateIntent.putExtra(MoKeeUpdater.EXTRA_FINISHED_DOWNLOAD_ID, id);
								updateIntent.putExtra(MoKeeUpdater.EXTRA_FINISHED_DOWNLOAD_PATH, completedFileFullPath);
							} else
							{
				...
				if (app.isMainActivityActive())
					...
					} else
					{
						...
						Notification.Builder builder = new Notification.Builder(context).setSmallIcon(R.drawable.cm_updater)
						...
						if (failureMessageResId >= 0)
							...
							{
								String updateUiName = MokeeUpdateInfo.extractUiName(updateFile.getName());
